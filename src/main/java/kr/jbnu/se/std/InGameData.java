    ...
    public static void saveScore(String email, int scoreData) {
        ...
        usersRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                Integer topScore = dataSnapshot.child(USER_INFO_PATH + sanitizedEmail + "/topScore").getValue(Integer.class);
                String nickname = dataSnapshot.child(USER_INFO_PATH + sanitizedEmail + "/nickname").getValue(String.class);
                Map<String, Object> userUpdates = new HashMap<>();

                // topScore와 currentScore 비교
                if (topScore == null || score > topScore) {
                    // currentScore가 topScore보다 크면 topScore를 currentScore로 업데이트
                    userUpdates.put(USER_INFO_PATH + sanitizedEmail + "/topScore", score);
                    userUpdates.put(USER_INFO_PATH + sanitizedEmail + "/currentScore", score);
